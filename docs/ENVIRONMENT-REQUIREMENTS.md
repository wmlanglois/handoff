...
The current registry explicitly does not establish general authenticated-endpoint support. Do not remove server authentication to make registration work; treat missing client auth support as a compatibility gap. Setup also does not automatically map primary/skeptic roles: follow the current README and configuration template. These commands do not implement the complete proposed readiness journey.
