...
If the user supplied JSON contracts, use `--plan-file` instead of regenerating them. Structured Markdown items can use `backlogc.py`; read [the overnight guide](OVERNIGHT.md) for its exact format and partial-compilation limits. Preserve arbitrary prose and resolve structure without dropping requirements. Machine-checkable seeded goal criteria now reach the planner with their IDs (#6). Do not repeat intake as a substitute for preserving the supplied design.

Interactive mode stops for actual approval. Explicit standing delegation is different: after confirmed intake, use `autonomous --delegate <name>` for missing-scope generation and scope, plan, and derived-map approvals, including resume at a proposed map. Do not repeatedly ask for authority the user already gave, or record invented answers/sign-offs. Live missing-scope-to-dispatch validation remains #13; the connection is implemented. Delegation does not authorize unrelated actions, waived checks, new spending limits, or human-only sign-offs.

E3=D shares C's within-goal review behavior. For cross-milestone execution, use the implemented `overnight.py` wrapper under an authorized backlog and cumulative budget (#7). Read its fan-in and blocked-resume limits in [OVERNIGHT](OVERNIGHT.md). Establish predictable decisions before a walkaway run; distinguish an implemented offline path from a live demonstration. Preserve admissible authorized progress; do not destroy a working proposal merely to demand a larger one.
...
5. For persistent services, declare an approved readiness probe; #41 prevents timeout crashes but the subsequent mutant gate is still exit-based. Do not equate a green health endpoint with project promotion. One launchable journey in a fresh process, after the wave, not only when every packet criterion is already met. Packet oracles passing do not pass this gate. DONE also requires that same command to fail when the delivered behavior is replaced by stubs. Promotion records that passing journey.
6. Evidence-based follow-up. An owned repair enters the ordinary queue with its evidence and staged dependencies. A separate authorized overnight backlog can admit the next milestone; a standalone autonomous goal does not invent or authorize one. Journey Spark is only live when `FLEET_SPARK_LIVE=1`; otherwise its `none found` record is not a model review. When invoked, it receives only the accessible evidence; it is not the acceptance authority.
...
CLI autonomous/overnight launch from frozen harness snapshots (#9); the separately launched tool service, settings and remote servers remain external. Keep one controller and stable run-state paths. The manual `fingerprint` still checks only `run/`. Do not bypass snapshots or edit live external services to sneak a fix into a trial.
